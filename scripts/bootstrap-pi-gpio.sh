...
# rpi-lgpio depends on lgpio, but pip would re-fetch the PyPI source tarball
# (no cp313 wheel) and fail on swig. lgpio is already installed — skip deps.
echo "Installing rpi-lgpio (no-deps — lgpio wheel already present) ..."
poetry run pip install --force-reinstall --no-deps rpi-lgpio

echo "Verifying ..."
poetry run python -c "import lgpio; import RPi.GPIO as GPIO; import board; print('OK')"
echo "Done. Run: poetry install"
