...
function animateToServos(targetAngles, durationMs = 500) {
  if (state.reachAnimFrame) cancelAnimationFrame(state.reachAnimFrame);
  const start = { ...state.servo };
  const keys = Object.keys(targetAngles);
  const t0 = performance.now();
  function step(now) {
    const t = Math.min(1, (now - t0) / durationMs);
    for (const j of keys) {
      if (targetAngles[j] == null) continue;
      const v = start[j] + (targetAngles[j] - start[j]) * t;
      const clamped = setServoAngle(j, v);
...
export async function goToReachCoords() {
  if (!state.apiReady) {
    setReachGoStatus("start API: poetry run roboarm-sim", "err");
    return;
  }
  try {
    ...
    if (!sol?.tip) {
      setReachGoStatus("IK failed — is roboarm-sim running?", "err");
      return;
    }
    const tip = sol.tip;
    const err = sol.errorMm ?? REACH_GO_TOL_MM + 1;

    if (!sol.reachable || err > REACH_GO_TOL_MM) {
      const sugg = await apiSuggestPitch(parsed.x, parsed.y, parsed.z);
    ...
    animateToServos(sol.servoAngles);
...
function configureReachZSlider() {
  ...
  state.reachZTolerance = Math.max(8, (zMax - zMin) / 24);
...
export async function buildReachability() {
  ...
  if (!state.apiReady) {
    const countEl = document.getElementById("reachZCount");
    if (countEl) countEl.textContent = "reach cloud needs API — poetry run roboarm-sim";
    return;
  }
  const data = await apiFetch("/api/reach/samples");
  const vol = [];
  const floor = [];
  state.reachAllSamples = [];
