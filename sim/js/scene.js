...
import { makeLabel } from "./three-utils.js";
...
const baseHLabel = makeLabel("shoulder " + L.shoulderHeight + " " + state.UNIT);
...
const upperArmLabel = makeLabel("upper arm " + L.upperArm + " " + state.UNIT);
...
const forearmLabel = makeLabel("forearm " + L.forearm + " " + state.UNIT);
...
const wristPitchLabel = makeLabel("wrist pitch");
...
const wristRotLabel = makeLabel("wrist_rot +" + L.wristRotOffset + " " + state.UNIT);
...
const gripperMotorLabel = makeLabel("gripper @" + gripAt + " " + state.UNIT);
...
if (gOff !== 0) {
  ...
  const offLabel = makeLabel("offset " + gOff + " " + state.UNIT);
  offLabel.position.set(gripAt + 6, 8, gOff / 2);
  handGroup.add(offLabel);
}
const handLabel = makeLabel("hand " + L.hand + " " + state.UNIT);
