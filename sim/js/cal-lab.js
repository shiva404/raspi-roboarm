/** Angle mapping lab — min/max vs zero_deg/sign tuning. */

import { CAL_JOINTS } from "./constants.js";
import { state } from "./state.js";
import { clampAngle, setServoAngle } from "./config.js";
...
export function applyJointLimits(j, min, max) {
  ...
  max = Math.round(max);
  if (min >= max) return;
  state.CONFIG.servos[j].min = min;
  state.CONFIG.servos[j].max = max;
  ...
  const clamped = setServoAngle(j, state.servo[j]);
...
function resetCalJoint(j) {
  ...
  state.CONFIG.servos[j].min = snap.servos[j].min;
  state.CONFIG.servos[j].max = snap.servos[j].max;
...
export function buildCalControls() {
  ...
  document.getElementById("cal_min").addEventListener("input", (e) => {
    ...
    applyJointLimits(j, Number(e.target.value), state.CONFIG.servos[j].max);
  });
  document.getElementById("cal_max").addEventListener("input", (e) => {
    document.getElementById("cv_max").textContent = e.target.value + "°";
    applyJointLimits(j, state.CONFIG.servos[j].min, Number(e.target.value));
  });
...
export function refreshCalLab() {
  ...
  const j = state.calJoint;
  const commanded = Number(document.getElementById("s_" + j)?.value ?? state.servo[j]);
  const clamped = clampAngle(commanded, state.CONFIG.servos[j]);
  ...
  document.getElementById("calPipe").innerHTML =
    ...
    '<div class="hint">min=' + state.CONFIG.servos[j].min + ' max=' + state.CONFIG.servos[j].max +
